...
class ChartGenerator:
    ...
    def create_follower_distribution_chart(self, analyses: List[FollowerAnalysis]) -> str:
        ...
        # Histogram
        ax1.hist(follower_counts, bins=20, alpha=0.7, color='skyblue', edgecolor='black')
    ...
    def create_location_analysis_chart(self, analyses: List[FollowerAnalysis]) -> str:
        ...
        locations = [analysis.profile.location for analysis in analyses 
    ...
    def create_engagement_analysis_chart(self, analyses: List[FollowerAnalysis]) -> str:
        ...
        for analysis in analyses:
            if analysis.recent_tweets:
                data.append({
                    ...
                    'avg_retweets': np.mean([tweet.retweet_count for tweet in analysis.recent_tweets]),
                    'avg_likes': np.mean([tweet.favorite_count for tweet in analysis.recent_tweets])
                })
        
        if not data:
            return self._create_no_data_chart("No tweet data available for engagement analysis")
        ...
        ax2.hist(df['avg_retweets'], bins=15, alpha=0.7, color='green', edgecolor='black')
    ...
    def create_interactive_dashboard_data(self, analyses: List[FollowerAnalysis]) -> Dict[str, Any]:
        ...
        for analysis in analyses:
            data = {
                ...
                'recent_tweets_count': len(analysis.recent_tweets) if analysis.recent_tweets else 0,
                'liked_tweets_count': len(analysis.liked_tweets) if analysis.liked_tweets else 0
            }
            
            if analysis.recent_tweets:
                data['avg_retweets'] = np.mean([t.retweet_count for t in analysis.recent_tweets])
                data['avg_likes'] = np.mean([t.favorite_count for t in analysis.recent_tweets])
            else:
                data['avg_retweets'] = 0
                data['avg_likes'] = 0
